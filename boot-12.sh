...
# -----------------------------------------------------------------
# 1️⃣  Define absolute locations (makes the script portable & clear)
# -----------------------------------------------------------------
PROJECT_ROOT="$(pwd)"                     # directory where the script is executed
BACKEND_DIR="${PROJECT_ROOT}/backend"     # absolute path to the backend folder
VENV_DIR="${BACKEND_DIR}/.venv"           # absolute path to the virtual‑env
REQ_FILE="${BACKEND_DIR}/requirements.txt"

# -----------------------------------------------------------------
# 2️⃣  Create backend folder (if missing) and move into it
# -----------------------------------------------------------------
mkdir -p "${BACKEND_DIR}"
cd "${BACKEND_DIR}"

# -----------------------------------------------------------------
# 3️⃣  Create the virtual‑env **only if it does not exist**
# -----------------------------------------------------------------
if [[ ! -d "${VENV_DIR}" ]]; then
...
# -----------------------------------------------------------------
# 4️⃣  Activate the virtual‑env **using the absolute path**
# -----------------------------------------------------------------
source "${VENV_DIR}/bin/activate"
ok "Activated virtualenv: ${VENV_DIR}"

# -----------------------------------------------------------------
# 5️⃣  Upgrade pip (explicitly using the venv’s pip)
# -----------------------------------------------------------------
step "Upgrading pip (venv version)"
"${VENV_DIR}/bin/pip" install --upgrade pip
ok "pip upgraded to $(pip --version | awk '{print $2}')"

# -----------------------------------------------------------------
# 6️⃣  Install required packages (list is now a single variable for easy editing)
# -----------------------------------------------------------------
PKGS=(
  fastapi
  uvicorn
...
run_with_spinner "Installing backend Python packages (venv)" \
  "${VENV_DIR}/bin/pip" install "${PKGS[@]}"

# -----------------------------------------------------------------
# 7️⃣  Freeze exact versions to a **requirements.txt** located in the backend root
# -----------------------------------------------------------------
"${VENV_DIR}/bin/pip" freeze > "${REQ_FILE}"
ok "Backend dependencies installed and frozen to ${REQ_FILE}"
...
# -------------------------------------------------
step "Adding public/README.md placeholder"
if [[ ! -d public ]]; then
  mkdir -p public
