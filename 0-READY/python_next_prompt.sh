...
HEADER="Next.js / FastAPI Full-Stack Starter"
...
echo -e "\033[0;37m     • FastAPI with Python 3.12 on the backend\033[0m"
...
echo -e "\033[0;37m     2. Verify shell configuration and ensure NVM/PNPM/pyenv paths are set\033[0m"
...
echo -e "\033[0;37m     7. Install Python 3.12 via pyenv (if not already installed)\033[0m"
echo -e "\033[0;37m     8. Install uv (Python package installer)\033[0m"
echo -e "\033[0;37m     9. Prompt for project name or PATH and create project directory\033[0m"
echo -e "\033[0;37m     10. Scaffold backend (FastAPI, SQLAlchemy) and frontend (Next.js) with tooling\033[0m"
echo -e "\033[0;37m     11. Set up Git repository and initial .gitignore\033[0m"
...
if [[ "$(printf '%s\n' "$MIN_PY" "$PYVER" | sort -V | head -n1)" != "$MIN_PY" ]]; then
  if ! command -v pyenv &>/dev/null; then
    echo -e "${YELLOW}pyenv will be installed.${NC}"
    read -rp "Do you want to continue? (y/N): " confirm
    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
      echo -e "\n${RED}Installation cancelled by user${NC}"
      exit 1
  ...
  echo -e "${YELLOW}Python $MIN_PY will be installed.${NC}"
  read -rp "Do you want to continue? (y/N): " confirm
  if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
    echo -e "\n${RED}Installation cancelled by user${NC}"
    exit 1
...
if ! command -v uv &> /dev/null; then
  echo -e "${YELLOW}uv (Python package installer) will be installed.${NC}"
  read -rp "Do you want to continue? (y/N): " confirm
  if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
    echo -e "\n${RED}Installation cancelled by user${NC}"
    exit 1
