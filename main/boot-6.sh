...
# =================================================
# Verify PostgreSQL client & connection
# ===============================================

# Install psql client if missing
if [[ "$PKG" == "apt" ]]; then
  run_with_spinner "Installing PostgreSQL client (apt)" \
    sudo apt-get install -y postgresql-client
elif [[ "$PKG" == "brew" ]]; then
  run_with_spinner "Installing PostgreSQL client (brew)" \
    brew install libpq
fi
...
if [[ -z "${DATABASE_URL:-}" ]]; then
  ...
  if psql "$DATABASE_URL" -c '\q' >/dev/null 2>&1; then
    ...
    echo -e "\n${RED}✖ Unable to connect to Postgres${NC}"
    exit 1
  fi
fi
...
# ---- Postgres availability check (environment loading only)
if [[ -f .env ]]; then
  set -a
  source .env
  set +a
