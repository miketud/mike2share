...
if command -v python3 >/dev/null; then
  ...
  if [[ "$(printf '%s\n' "$MIN_PY" "$PYVER" | sort -V | head -n1)" == "$MIN_PY" ]]; then
    echo "System python $PYVER satisfies ≥$MIN_PY"
    HAVE_PYTHON=true
  else
    HAVE_PYTHON=false
  fi
...
if [[ ${HAVE_PYTHON:-false} != true ]]; then
  ...
  eval "$(pyenv init -)"
  # pick the latest 3.14.x released by pyenv
  LATEST_3_14=$(pyenv install -l | grep -E '^ 3\.14\.[0-9]+$' | tail -1 | tr -d ' ')
  if [[ -z "$LATEST_3_14" ]]; then
    echo "❌ No 3.14 version found – aborting"
    exit 1
  fi
  pyenv install -s "$LATEST_3_14"
  pyenv global "$LATEST_3_14"
  echo "pyenv Python $LATEST_3_14 ready (≥$MIN_PY)"
fi
