...
# *** NEW *** create an empty folder for shared UI components
...
run_with_spinner "Installing frontend dependencies" \
  npm install axios @tanstack/react-query zustand classnames dayjs framer-motion \
  @vanilla-extract/css @vanilla-extract/recipes @vanilla-extract/sprinkles \
  gsap react-intersection-observer @use-gesture/react react-use \
  lottie-react react-icons lenis scroll-snap split-type howler
...
# ---- stylelint
run_with_spinner "Installing stylelint" \
